...
- [x] **Input validation layer** (e.g. `zod` or `express-validator`) applied at route boundaries. — Done 2026-08-19: `zod` + `middleware/validate.js` (one shared middleware, applied per-route via `validate(schema)`), with schemas in `validation/` mirroring `routes/`. Schemas call the same tested functions from `utils/validation.js` via `.refine()` rather than re-implementing rules. Fixed 3 real gaps found along the way, all now covered by tests: (1) `POST /api/alerts` never actually checked `zip`'s format, only presence; (2) a malformed `:id` on `DELETE`/`PATCH /api/alerts/:id` threw an uncaught Mongoose `CastError` → opaque `500`, now a clean `400`; (3) an invalid `notificationMethod` relied entirely on Mongoose's `enum` validator (via `runValidators`) → also an opaque `500`, now caught at the boundary. 5 new tests added (48 total); full manual regression pass confirmed real NYC API calls, rate limiting, auth, and the cron secret all still behave identically.
